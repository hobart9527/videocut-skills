/**
 * Web review server
 * Refactored from review_server.js to work with the CLI
 */
...
/**
 * Generate review.html (static file, no server needed to view)
 * @param {Array} words - subtitles_words.json
 * @param {Array} autoSelected - indices pre-selected by AI
 * @param {string} outputPath - where to write review.html
 */
function generateReviewHTML(words, autoSelected, outputPath) {
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    #time { font-family: monospace; font-size: 16px; color: #888; }
    .main { display: flex; flex: 1; overflow: hidden; }
    .left-panel {
      width: 420px; flex-shrink: 0; padding: 10px;
      display: flex; flex-direction: column; gap: 10px;
      border-right: 1px solid #333;
    }
    #player { width: 100%; border-radius: 4px; background: #000; }
    .help {
      font-size: 12px; color: #999; background: #252525;
      padding: 10px; border-radius: 6px; line-height: 1.6;
    }
    .help b { color: #fff; }
    .right-panel { flex: 1; overflow-y: auto; padding: 15px 20px; }
    ...
    .gap {
      display: inline-block; background: #333; color: #888;
    ...
    .stats { padding: 10px; background: #252525; border-radius: 4px; font-size: 14px; }
    .loading-overlay {
    ...
    .loading-progress-bar {
      height: 100%; background: linear-gradient(90deg, #9C27B0, #E91E63);
...
<body>
  ...
  <div class="toolbar">
    <div class="buttons">
      ...
      <button class="danger" onclick="clearAll()">清空选择</button>
  ...
  <div class="main">
    <div class="left-panel">
      <video id="player" src="${videoName}" preload="auto"></video>
      <div class="stats" id="stats"></div>
      <div class="help">
        <div><b>鼠标：</b>单击跳转 | 双击选中 | Shift+拖动批量</div>
        <div><b>键盘：</b>空格播放 | 左右跳1s | Shift+左右跳5s</div>
  ...
  <script>
    ...
    function formatDuration(sec) {
      ...
      return m > 0 ? m + '分' + s + '秒 (' + sec.toFixed(1) + 's)' : s + '秒';
    }
    ...
    function render() {
      ...
      words.forEach((word, i) => {
        ...
        if (word.isGap) {
          div.textContent = '静 ' + (word.end - word.start).toFixed(1) + 's';
        } else {
          div.textContent = word.text;
        }
        div.dataset.index = i;
        div.onclick = (e) => { if (!isSelecting) player.currentTime = word.start; };
        div.ondblclick = () => toggle(i);
        div.onmousedown = (e) => {
          if (e.shiftKey) { isSelecting = true; selectStart = i; selectMode = selected.has(i) ? 'remove' : 'add'; e.preventDefault(); }
        };
    ...
    content.addEventListener('mousemove', e => {
      ...
      for (let j = min; j <= max; j++) {
        if (selectMode === 'add') { selected.add(j); elements[j].classList.add('selected'); elements[j].classList.remove('ai-selected'); }
        else { selected.delete(j); elements[j].classList.remove('selected'); if (autoSelected.has(j)) elements[j].classList.add('ai-selected'); }
      }
      updateStats();
    });
    document.addEventListener('mouseup', () => { if (isSelecting) rebuildSkipIntervals(); isSelecting = false; });

    function toggle(i) {
      if (selected.has(i)) { selected.delete(i); elements[i].classList.remove('selected'); if (autoSelected.has(i)) elements[i].classList.add('ai-selected'); }
      else { selected.add(i); elements[i].classList.add('selected'); elements[i].classList.remove('ai-selected'); }
      rebuildSkipIntervals(); updateStats();
    }
    ...
    // Web Audio API for sample-accurate muting
    ...
    function rebuildSkipIntervals() {
      ...
      while (i < sorted.length) {
        ...
        let j = i + 1;
        while (j < sorted.length && words[sorted[j]].start - end < 0.1) { end = words[sorted[j]].end; j++; }
      ...
      if (!player.paused) {
        for (const iv of skipIntervals) {
          if (t >= iv.start && t < iv.end) {
            if (!skipLock) { skipLock = true; gainNode.gain.setValueAtTime(0, audioCtx.currentTime); player.currentTime = iv.end; }
            return;
          }
        }
        if (skipLock) { skipLock = false; gainNode.gain.setValueAtTime(1, audioCtx.currentTime); }
      }
      timeDisplay.textContent = formatTime(t) + ' / ' + formatTime(player.duration || 0);
      let curr = -1;
      for (let i = 0; i < words.length; i++) { if (t >= words[i].start && t < words[i].end) { curr = i; break; } }
      if (curr !== lastHighlight) {
        if (lastHighlight >= 0 && lastHighlight < elements.length) elements[lastHighlight].classList.remove('current');
        if (curr >= 0) { elements[curr].classList.add('current'); elements[curr].scrollIntoView({ behavior: 'smooth', block: 'center' }); }
        lastHighlight = curr;
      }
    }
    requestAnimationFrame(tick);
    ...
    function copyDeleteList() {
      ...
      sortedSelected.forEach(i => { segments.push({ start: words[i].start, end: words[i].end }); });
      const merged = [];
    ...
    function clearAll() {
      selected.clear();
      elements.forEach((el, i) => { el.classList.remove('selected'); if (autoSelected.has(i)) el.classList.add('ai-selected'); });
      rebuildSkipIntervals(); updateStats();
    }
...
/**
 * Start review server and wait for user confirmation
 * @param {string} reviewDir - directory containing review.html and video.mp4
 * @param {Object} options
 * @returns {Promise<Array>} - delete segments list
 */
function startServer(reviewDir, options = {}) {
  const PORT = options.port || 8899;
  const VIDEO_FILE = options.videoPath || path.join(reviewDir, 'video.mp4');
  ...
  return new Promise((resolve, reject) => {
    ...
    const server = http.createServer((req, res) => {
      ...
      if (req.method === 'POST' && req.url === '/api/cut') {
        ...
        req.on('end', () => {
          try {
            ...
            fs.writeFileSync(path.join(reviewDir, 'delete_segments.json'), JSON.stringify(segments, null, 2));
            console.log('Saved ' + segments.length + ' segments for deletion');

            // Execute cut
            const outputFile = path.join(reviewDir, 'output_cut.mp4');
            const { executeCut } = require('../utils/ffmpeg');
            executeCut(VIDEO_FILE, segments, outputFile);
            ...
            res.end(JSON.stringify({
              ...
              deletedDuration: (originalDuration - newDuration).toFixed(2),
              savedPercent: (((originalDuration - newDuration) / originalDuration) * 100).toFixed(1),
            }));

            deleteList = segments;
            setTimeout(() => { server.close(); resolve(deleteList); }, 1000);
          } catch (err) {
      ...
      filePath = path.join(reviewDir, filePath);
    ...
    server.listen(PORT, () => {
      ...
      console.log('\nAfter reviewing and clicking "执行剪辑", the server will close.\n');
    });
    ...
    setTimeout(() => {
      if (!deleteList) {
        server.close();
        reject(new Error('Review timed out (30 minutes)'));
      }
    }, 30 * 60 * 1000);
  });
}
...
/**
 * Standalone review server command
 */
function reviewServer(options = {}) {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.method === 'POST' && req.url === '/api/cut') {
      ...
      req.on('end', () => {
        try {
          const segments = JSON.parse(body);
          fs.writeFileSync(path.join(dir, 'delete_segments.json'), JSON.stringify(segments, null, 2));
          console.log('Saved ' + segments.length + ' segments');
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ success: true, message: 'Saved delete_segments.json' }));
        } catch (err) {
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ success: false, error: err.message }));
        }
  ...
  server.listen(PORT, () => {
    console.log('\n🎬 Review server started (persistent)');
    console.log('📍 http://localhost:' + PORT);
...
module.exports = { generateReviewHTML, startServer, reviewServer };
