...
2. **剪口播产出的 `_cut.mp4`**：在当前 output 目录下查找 `剪口播/3_审核/*_cut.mp4`
...
**关键**：字幕应该基于**剪辑后的视频**生成，而不是原始视频。因为剪口播会删除口误和静音段，原始视频的时间戳与剪辑后不匹配。
...
- 新增 Step 0 自动查找视频逻辑：优先找 `剪口播/3_审核/*_cut.mp4`
